  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: "ANTHROPIC_API_KEY is not set. Add it to your deployment environment variables (Cloudflare Pages: Settings → Environment Variables; Vercel: Settings → Environment Variables) and redeploy." },
        { status: 500 }
      );
    }
